...
- [ ] **Settings sub-resource stores (forwarding, team, security devices)** — `SettingsView.vue` fetches `forwarding`, `team`, and `securityDevices` directly into local refs with no caching layer, unlike `labels`/`threads`/`rules` which already have stores. Extract each into its own store (or one `settings` store with sub-state) so switching tabs doesn't always refetch. (`domains`/`aliases`/`exchanges` are covered by the sender-identities item below instead.)

- [ ] **Wire `useSenderIdentitiesStore` into Settings, `SenderInfoPopup`, and `useSearch`** — `src/stores/senderIdentities.ts` (added in PR #69 for the draft composer's From picker) already caches `domains`/`aliases`/`exchanges` account-wide: cache-first render, `ensureLoaded()` kicks a deduped background fetch (one per account per session) without blocking whatever's on screen. Nothing else uses it yet, and this isn't a drop-in swap for any of them — every consumer still needs its own `ensureLoaded()` call wired up (mount-time background fetch, same as `DraftSignalCard`), not just a read from the store:
  - **Read-only — straightforward:** `SenderInfoPopup.vue` (fetches `listAliases` to look up one alias's policy) and `useSearch.ts` (refetches `listAliases` fresh on *every* search query today, with zero caching). Swap each local fetch for `senderIdentities.aliases` + `ensureLoaded()`.
  - **Settings (`SettingsView.vue`) — reads *and* writes:** currently fetches `domains`/`aliases`/`exchanges` into local refs per-tab (see the superseded note this replaces). Since we're touching every call site anyway, hook up the write side too: `addDomain`; `createAlias`/`updateAlias`/`deleteAlias`; `createExternalExchange`/`patchExternalExchange`/`deleteExternalExchange`. Each mutation needs to write its result back into the store (or trigger a re-`fetchAll()`) — otherwise Settings and the draft composer can disagree after an edit made in either one. This is most of the work; the reads are nearly free once the store already exists.
  - **Alias senders** (`listAliasSenders`/`addAliasSender`/`updateAliasSender`/`removeAliasSender`, used by `SenderInfoPopup.vue` and Settings' alias detail) is a separate per-alias sub-resource that doesn't fit `senderIdentities`'s flat `Record<accountId, T>` shape — decide whether it needs its own store keyed by alias, or stays local.
