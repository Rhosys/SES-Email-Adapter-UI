...
<template>
  <RouterLink :to="{ name: 'thread-detail', params: { id: thread.threadId } }" class="min-w-0 flex-1">
    <div class="flex items-center gap-2">
      <span class="w-40 shrink-0 truncate text-sm font-semibold text-ctp-text">{{ thread.senderAddress || 'Unknown' }}</span>
      <span class="flex-1 truncate text-sm text-ctp-subtext1">{{ thread.subject || thread.summary }}</span>
