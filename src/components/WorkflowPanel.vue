...
<template>
  <div v-if="resolvedGroup && resolvedGroup.entries.length > 1" class="rounded-lg border border-ctp-surface1 bg-ctp-mantle">
    <template v-for="(entry, idx) in resolvedGroup.entries" :key="idx">
      <AuthPanel v-if="resolvedGroup.workflow === 'auth' && isAuthVisible(entry, idx)" :data="narrowWorkflowData('auth', entry)" :actions="actionsForEntry(idx)" :received-at="receivedAt" :compact="compact" />
  ...
  <template v-else-if="resolvedGroup && resolvedGroup.entries.length === 1">
    <AuthPanel v-if="resolvedGroup.workflow === 'auth' && isAuthVisible(resolvedGroup.entries[0], 0)" :data="narrowWorkflowData('auth', resolvedGroup.entries[0])" :actions="entryActions" :received-at="receivedAt" :compact="compact" />
    <ConversationPanel v-else-if="resolvedGroup.workflow === 'conversation'" :data="narrowWorkflowData('conversation', resolvedGroup.entries[0])" :compact="compact" />
    <CrmPanel v-else-if="resolvedGroup.workflow === 'crm'" :data="narrowWorkflowData('crm', resolvedGroup.entries[0])" :compact="compact" />
    <PackagePanel v-else-if="resolvedGroup.workflow === 'package'" :data="narrowWorkflowData('package', resolvedGroup.entries[0])" :compact="compact" />
    <TravelPanel v-else-if="resolvedGroup.workflow === 'travel'" :data="narrowWorkflowData('travel', resolvedGroup.entries[0])" :compact="compact" />
    <PaymentsPanel v-else-if="resolvedGroup.workflow === 'payments'" :data="narrowWorkflowData('payments', resolvedGroup.entries[0])" :compact="compact" />
    <AlertPanel v-else-if="resolvedGroup.workflow === 'alert'" :data="narrowWorkflowData('alert', resolvedGroup.entries[0])" :actions="entryActions" :compact="compact" />
    <ContentPanel v-else-if="resolvedGroup.workflow === 'content'" :data="narrowWorkflowData('content', resolvedGroup.entries[0])" :compact="compact" />
    <StatusPanel v-else-if="resolvedGroup.workflow === 'status'" :data="narrowWorkflowData('status', resolvedGroup.entries[0])" :compact="compact" />
    <HealthcarePanel v-else-if="resolvedGroup.workflow === 'healthcare'" :data="narrowWorkflowData('healthcare', resolvedGroup.entries[0])" :compact="compact" />
    <JobPanel v-else-if="resolvedGroup.workflow === 'job'" :data="narrowWorkflowData('job', resolvedGroup.entries[0])" :actions="entryActions" :compact="compact" />
    <SupportPanel v-else-if="resolvedGroup.workflow === 'support'" :data="narrowWorkflowData('support', resolvedGroup.entries[0])" :compact="compact" />
    <EventsPanel v-else-if="resolvedGroup.workflow === 'events'" :data="narrowWorkflowData('events', resolvedGroup.entries[0])" />
    <TestPanel v-else-if="resolvedGroup.workflow === 'test'" :data="narrowWorkflowData('test', resolvedGroup.entries[0])" :compact="compact" />
  </template>
</template>
