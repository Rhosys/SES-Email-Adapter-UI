...
import { useMoveToInbox } from '@/composables/useThreadQueries'
...
function unarchiveThread(close?: () => void) {
  close?.()
  moveToInboxMutation.mutate(props.thread.threadId)
}
</script>
...
<template>
  ...
  >
    <div
      class="group relative flex items-center gap-2 border-b border-ctp-surface1 px-3 py-3.5 transition-colors hover:bg-ctp-surface0 sm:py-2.5"
